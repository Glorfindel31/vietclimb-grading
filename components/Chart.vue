	...
	<Card :class="cn(props.class)">
		...
		<CardContent class="w-full">
			<VisXYContainer :margin="{ left: 10, right: 10 }" :data="data" :style="{
				...
				<VisStackedBar :x="(___d: RouteCount, i: number) => i" :y="(d: RouteCount) => d.amount"
					color="var(--theme-primary)" :rounded-corners="4" :bar-padding="0.05" />
				<VisAxis type="x" :num-ticks="data.length"
					:tick-format="(index: number) => 'Lvl' + ' ' + data[index]?.level" tickTextColor="var(--theme-text)"
					:grid-line="false" :tick-line="true" />
				<VisAxis type="y" :num-ticks="data.length" :grid-line="false" :tick-line="false" :domain-line="false"
